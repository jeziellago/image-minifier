...
class QualityTransformation(private val quality: Int) : ImageTransformation {

    override fun apply(source: File, shouldCheckForRotation: Boolean): File {
        return source.apply {
            val bitmap = source.getAsBitmap(shouldCheckForRotation)
            outputStream().use { fos ->
                with(bitmap) {
                    compress(getExtensionAsBitmapFormat(), quality, fos)
                    recycle()
                }
            }
        }
